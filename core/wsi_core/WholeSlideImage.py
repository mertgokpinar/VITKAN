...
import h5py
import math
from core.wsi_core.wsi_utils import savePatchIter_bag_hdf5, initialize_hdf5_bag, coord_generator, save_hdf5, sample_indices, screen_coords, isBlackPatch, isWhitePatch, to_percentiles
import itertools
from core.wsi_core.util_classes import isInContourV1, isInContourV2, isInContourV3_Easy, isInContourV3_Hard, Contour_Checking_fn
from core.clam_utils.file_utils import load_pkl, save_pkl
...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
        ...
        contours, hierarchy = cv2.findContours(img_otsu, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)[-2:] # Find contours 
    ...
    def visWSI(self, vis_level=0, color = (0,255,0), hole_color = (0,0,255), annot_color=(255,0,0), 
        ...
        #img = np.resize(img,(img.shape[0],img.shape[1],4))
    ...
    def visHeatmap(self, scores, coords, vis_level=-1, 
        ...
        if not blank_canvas:
            ...
            #img = np.array(self.wsi.read_region(top_left, vis_level, region_size).convert("RGB"))
            img = self.wsi.read_region(top_left, vis_level, region_size).convert("RGB")
