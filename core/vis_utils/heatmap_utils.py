...
from core.clam_utils.wsi_dataset import Wsi_Region
import h5py
from core.wsi_core.WholeSlideImage import WholeSlideImage
...
def compute_from_patches(wsi_object, model=None, feature_extractor=None, omic_input = None, batch_size=512,  
    attn_save_path=None, ref_scores=None, feat_save_path=None, opt = None, **wsi_kwargs):    
    ...
    for idx, (roi, coords) in enumerate(roi_loader):
        ...
        with torch.no_grad():
            ...
            if attn_save_path is not None:
                omic = omic.expand(len(features),-1)
                A, pred = model(x_path=features,  x_omic=omic, vae_omic = 0, omic_model = opt.omic_model)
           
                # if A.size(0) > 1: #CLAM multi-branch attention
                #      A = A[pred]

                A = A.view(-1, 1).cpu().numpy()
                #A = A.mean(A[0], dim=0).cpu().numpy()

                if ref_scores is not None:
                     for score_idx in range(len(A)):
                        #  breakpoint()
                         A[score_idx] = score2percentile(A[score_idx], ref_scores.squeeze())
